...
public class Factorization {
    ...
    public void constructPolynomial() {
        ...
        al.add(new FiniteFieldElement(this.q, q - 1));

        // all the other coefficients are zero
        for (int i = 1; i < n; i++) {
            al.add(new FiniteFieldElement(this.q, BigInteger.ZERO));
        }

        // the n-th coefficient is one.
        al.add(new FiniteFieldElement(this.q, BigInteger.ONE));
        ...
        for (Coset cos: cosets) {
            ...
            for (int i = 0; i <= cos.getValues().last(); i++) {
                ...
                if (cos.getValues().contains(i)) {
                    v.add(new FiniteFieldElement(this.q, BigInteger.ONE));
                }
                else {
                    v.add(new FiniteFieldElement(this.q, BigInteger.ZERO));
                }
